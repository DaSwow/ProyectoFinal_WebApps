package Fabrica;

import Datos.*;
import Blog.Comun;
import java.util.List;

/**
 *
 * @author
 */
public class ctlComun {

    /**
     * Atributo instancia de la fachada de datos
     */
    private final iDatos datos = new FachadaDatos();

    /**
     *
     * @return Regresa todos los datos que se hayan encontrado en la base de
     * datos
     */
    public List<Comun> buscar() {
        return datos.buscaComun();
    }

    /**
     *
     * @param entidad
     */
    public void guardar(Comun entidad) {
        datos.guardarComun(entidad);
    }

    /**
     *
     * @param id
     * @param entidad
     */
    public void actualizar(int id, Comun entidad) {
        datos.actualizarComun(id, entidad);
    }

    /**
     * Realiza una consulta por ID en la base de datos
     *
     * @param id Recibe el ID de la entidad la cual se esta buscando en la base
     * de datos
     * @return Regresa la entidad encontrada con el mismo ID del parametro
     */
    public Comun buscar(int id) {
        return datos.buscarComun(id);
    }

    /**
     * Busca y elimina una entidad en la base de datos
     *
     * @param entidad
     */
    public void eliminar(Comun entidad) {
        datos.eliminarComun(entidad);
    }
}
